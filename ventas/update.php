...
try {
    ...
    $stmt = $conn->query("SELECT v.id, v.monto, v.fecha, p.id AS propiedad_id, p.ubicacion, c.id AS cliente_id, c.cliente 
                          FROM ventas AS v 
    ...
    if ($_POST) {
        ...
        $ventaController = new VentasController($conn);

        $ventaController->update($id, $propiedad_id, $cliente_id, $monto, $fecha);
    }
...
<body>
    ...
    <section class="" id="features">
            <div class="container mt-3">
                <div class="row">
                     <div class="col">
                         <h1 id="titulo">Modificar venta</h1>
                     </div>
                </div>
                <form method="post">
                    <div class="row mt-3 gx-5">
                        <div class="col-4">
                            ...
                            <select name="propiedad_id" class="form-control">
                                <?php
                                $propiedades = $conn->query("SELECT id, ubicacion FROM propiedades");
                                while ($propiedad = $propiedades->fetch(PDO::FETCH_ASSOC)) {
                                    $selected = ($propiedad['id'] == $venta['propiedad_id']) ? 'selected' : '';
                            ...
                            <div class="col-3">
                                ...
                                <select name="cliente_id" class="form-control">
                                    <?php
                                    $clientes = $conn->query("SELECT id, cliente FROM clientes");
                        ...
                        <div class="col-2">
                            <label>Monto</label>
                            <input type="text" name="monto" class="form-control" value="<?php echo($venta['monto']); ?>">
                        </div>
                        <div class="col-2">
                            <label>Fecha</label>
                            <input type="date" name="fecha" class="form-control" value="<?php echo($venta['fecha']); ?>">
                        </div>
                    </div>
                    <div class="row gx-5">
                        <div class="col right">
